...
# Program Copyright 1998 Ken Lunde. All Rights Reserved.
# Version 04/28/98
#
# Use "-h" for help
...
my ($bom, $variant, $hojo, $first, $version) = ("", 0, 1, 1, "04/28/98");
...
my $sjs = q{ # Shift-JIS encoding
  <[0-9A-F][0-9A-F][0-9A-F][0-9A-F]>         # Unicode tag
  | [\x09-\x0A\x0D\x20-\x7E]                 # ASCII and other one-byte
  | [\x81-\x9F\xE0-\xFC][\x40-\x7E\x80-\xFC] # Two-byte range
  | [\xA1-\xDF]                              # Half-width katakana
};
...
while (defined($line = <STDIN>)) {
  ...
  foreach $char (@chars) {
    $skip = 0;
    if ($char =~ /^<([0-9A-F][0-9A-F][0-9A-F][0-9A-F])>$/) {
      $char = $1;
...
handling:
  t = Output four-digit hexadecimal Unicode (big-endian UTF-16) tag <....>
...
sub BadCharInit ($) {
  ...
  print STDERR "Unicode tags" and $bad_tags = 1 if $option eq "t";
